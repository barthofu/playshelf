    ...
    macro_rules! get {
        ...
        ($route: literal, $arg_name: ident, $database: ident, $type: ident) => {
            ...
            pub async fn get(
                ...
                db
                    .run(move |c| crate::database::$database::find_one(c, $arg_name))
                    .await
                    .map(Json)
    ...
    macro_rules! get_all {      
        ...
        ($route: literal, $database: ident, $type: ident) => {
            ...
            pub async fn get_all(
                ...
                db
                    .run(|c| crate::database::$database::find_all(c))
                    .await
                    .map(Json)
    ...
    macro_rules! create {      
        ...
        ($route: literal, $database: ident, $type: ident, $new_type: ident) => {
            ...
            pub async fn create(
                ...
                db
                    .run(|c| crate::database::$database::create(c, new_data.into_inner()))
                    .await
                    .map(Json)
                    .map_err(|_| crate::utils::error::Error::Internal)
            }
    ...
    macro_rules! update {
        ...
        ($route: literal, $arg_name: ident, $database: ident, $type: ident, $update_type: ident) => {
            ...
            pub async fn update(
                ...
                db
                    .run(move |c| crate::database::$database::update(c, $arg_name, update_data.into_inner()))
                    .await
                    .map(Json)
                    .map_err(|e| match e {
    ...
    macro_rules! delete {      
        ...
        ($route: literal, $arg_name: ident, $database: ident) => {
            ...
            pub async fn delete(
                ...
                let success = db.run(move |c| crate::database::$database::delete(c, $arg_name)).await;
    ...
    macro_rules! associate {      
        ...
        (
            ...
            pub async fn $function_name(
                ...
                let first_resource = db.run(move |c| crate::database::$first_database::find_one(c, $first_arg_name)).await.map_err(|_| crate::utils::error::Error::NotFound)?;
                let second_resource = db.run(move |c| crate::database::$second_database::find_one(c, $second_arg_name)).await.map_err(|_| crate::utils::error::Error::NotFound)?;

                // checks if association exists
                let association_exists = db.run(move |c| crate::database::$first_database::has_track(&first_resource, &second_resource, c)).await;
                ...
                match association_exists {
                    ...
                    false => {
                        db
                            .run(move |c| crate::database::$first_database::create_track(&first_resource, &second_resource, c))
                            .await
                            .map(Json)
                            .map_err(|_| crate::utils::error::Error::Internal)
                    },
        ...
        (
            ...
            pub async fn $function_name(
                ...
                let success = db.run(move |c| {
                    ...
                    crate::database::$first_database::delete_track(&first_resource.unwrap(), &second_resource.unwrap(), c)
                }).await;
